...
class AdaptiveThresholdState:
    ...
    spki_max_step_ratio: float = 3.0  # clamp: a single peak can move SPKI up
                                       # by at most this multiple of current SPKI
    spki_decay_ratio: float = 0.6     # how hard to pull SPKI down toward a
                                       # failed search-back candidate
    next_searchback_retry_idx: int = 0  # throttle: don't decay every sample
    all_peaks_buffer: list = None     # [(idx, value), ...] — ALL local peaks,
                                       # NOT gated by TH2. See fix note where
                                       # this is populated: candidate_buffer
                                       # is gated by TH2 so when TH2 itself is
                                       # the problem (poisoned warm-up), it
                                       # stays permanently empty and the decay
                                       # recovery path can never see real
                                       # candidate amplitudes to pull toward.

    def __post_init__(self):
        if self.all_peaks_buffer is None:
            self.all_peaks_buffer = []
    ...
    def copy(self) -> "AdaptiveThresholdState":
        return AdaptiveThresholdState(
            ...
            self.spki_decay_ratio, self.next_searchback_retry_idx,
            list(self.all_peaks_buffer),
        )
...
def adaptive_threshold_step(mwi_value: float, slope_estimate: float,
    ...
    if peak_detected:
        ...
        # FIX: track ALL local peaks regardless of TH2, so the decay/
        # recovery path (below) has real candidate amplitudes to pull
        # SPKI toward even when TH2 itself is inflated and no peak can
        # ever clear it to enter candidate_buffer.
        new_state.all_peaks_buffer.append((peak_idx, peak_val))
        all_peaks_max_age = max(2 * new_state.default_rr_samples, new_state.n_refractory * 4, 1)
        new_state.all_peaks_buffer = [(i, v) for (i, v) in new_state.all_peaks_buffer
                                       if idx - i <= all_peaks_max_age]
        ...
        if ((idx - reference_idx) > new_state.gamma * effective_rr_mean and
            ...
            idx >= new_state.next_searchback_retry_idx and
            (len(new_state.candidate_buffer) > 0 or len(new_state.all_peaks_buffer) > 0)):
            best = (max(new_state.candidate_buffer, key=lambda t: t[1])
                    if new_state.candidate_buffer else None)
            if best is not None and best[1] > new_state.TH2:
                retro_idx = best[0]
                ...
                # FIX (2nd-order failure mode): repeated search-back timeouts
                # where even the strongest real candidate never clears TH2
                # mean the threshold itself is miscalibrated for this record
                # — typically a 2s warm-up window poisoned by a motion/
                # saturation artifact at record start (seen in high-std
                # records e.g. std>3). Nothing in the primary path can ever
                # bring SPKI back down on its own since SPKI only updates on
                # acceptance. So: pull SPKI toward the best real candidate
                # we've actually observed, throttled to roughly once per
                # assumed RR interval so it converges gradually rather than
                # collapsing on a single noise sample.
                #
                # FIX (bug in the first version of this fix): candidate_buffer
                # is gated by TH2, so when TH2 itself is inflated, no peak
                # ever enters candidate_buffer and this branch could never
                # even run. all_peaks_buffer tracks every local peak
                # unconditionally, so it's used here as the fallback source
                # of real candidate amplitudes to decay toward.
                decay_source = best if best is not None else (
                    max(new_state.all_peaks_buffer, key=lambda t: t[1])
                    if new_state.all_peaks_buffer else None)
                if decay_source is not None:
                    new_state.SPKI = ((1.0 - new_state.spki_decay_ratio) * new_state.SPKI
                                      + new_state.spki_decay_ratio * decay_source[1])
                    new_state.TH1 = new_state.NPKI + 0.25 * (new_state.SPKI - new_state.NPKI)
                    new_state.TH2 = 0.5 * new_state.TH1
                new_state.next_searchback_retry_idx = idx + max(new_state.default_rr_samples, 1)
                new_state.candidate_buffer = []
