...
class AdaptiveThresholdState:
    ...
    Plus refractory + last-R tracking for search-back.
...
def adaptive_threshold_init(n_refractory: int = 50, gamma: float = 1.66,
                            ...
                            startup_delay: int = 500) -> AdaptiveThresholdState:
    """Initialize adaptive threshold with startup delay.

    startup_delay: number of samples to skip before allowing peak detection.
    During this period, MWI values are collected for SPKI/NPKI initialization.
    Default = 500 samples = 2 seconds at 250 Hz.
    """
    state = AdaptiveThresholdState(
        ...
        # Set refractory to startup_delay to block all detections during startup
        refractory_samples_remaining=startup_delay,
        last_R_sample_idx=-1,
        last_R_slope=0.0,
    ...
    # Store startup MWI values for SPKI/NPKI initialization
    state._startup_mwi_values = []
    state._startup_delay = startup_delay
    return state
...
def adaptive_threshold_step(mwi_value: float, slope_estimate: float,
    ...
    # === STARTUP DELAY: collect MWI values, don't detect peaks ===
    # During the first startup_delay samples, collect MWI values for
    # SPKI/NPKI initialization. This prevents the MWI startup transient
    # from being accepted as a signal peak and locking SPKI at an
    # unreachable value.
    if hasattr(new_state, '_startup_mwi_values') and new_state._startup_delay > 0:
        if idx < new_state._startup_delay:
            # Still in startup — collect MWI value, don't detect
            new_state._startup_mwi_values.append(val)
            new_state.prev_mwi = val
            new_state.prev_mwi_idx = idx
            new_state.current_sample_idx = idx + 1
            # Keep refractory active during startup
            new_state.refractory_samples_remaining = max(new_state.refractory_samples_remaining, 1)
            return [], new_state
        elif idx == new_state._startup_delay:
            # Startup just ended — initialize SPKI/NPKI from collected values
            mwi_arr = np.array(new_state._startup_mwi_values)
            if len(mwi_arr) > 0:
                # Use 95th percentile for SPKI (robust to transient spikes)
                # and median for NPKI (noise floor)
                new_state.SPKI = float(np.percentile(mwi_arr, 95))
                new_state.NPKI = float(np.median(mwi_arr))
                if new_state.SPKI < 1e-9:
                    new_state.SPKI = 1.0
                new_state.TH1 = new_state.NPKI + 0.25 * (new_state.SPKI - new_state.NPKI)
                new_state.TH2 = 0.5 * new_state.TH1
            # Clear refractory — start detecting
    ...
    if peak_detected:
        ...
        if (new_state.last_R_sample_idx >= 0 and
            new_state.recent_rr_mean > 0 and
            (idx - new_state.last_R_sample_idx) > new_state.gamma * new_state.recent_rr_mean and
            ...
            if best[1] > new_state.TH2:
                ...
                new_state.SPKI = 0.125 * best[1] + 0.875 * new_state.SPKI
                new_state.TH1 = new_state.NPKI + 0.25 * (new_state.SPKI - new_state.NPKI)
                new_state.TH2 = 0.5 * new_state.TH1
        ...
        if (peak_val > new_state.TH1 and
            ...
            if twave_ok:
                ...
                new_state.SPKI = 0.125 * peak_val + 0.875 * new_state.SPKI
                new_state.TH1 = new_state.NPKI + 0.25 * (new_state.SPKI - new_state.NPKI)
                new_state.TH2 = 0.5 * new_state.TH1
                new_state.refractory_samples_remaining = new_state.n_refractory
