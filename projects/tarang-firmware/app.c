/*************************************************************
 * app.c — TARANG Clinical IoT Wearable Core Firmware (hardened)
...
void LDMA_IRQHandler(void)
{
    ...
    if (flags & (1u << LDMA_CH_ECG)) {
        ...
        if (Tarang_Pool[filling].meta.state != BUFFER_STATE_FREE) {
            ...
            /* LDMA chain cannot stall — pause LETIMER to protect PROCESSING data */
    ...
    /* IMU framed burst: optional completion via same ISR when one-shot armed */
...
void app_init(void)
{
    ...
    /* HFXO enabled only during EM0 compute — reverted before first EM2 entry */
    ...
    /* IMU LDMA is NOT started continuously — framed burst per ECG window */
    ...
    /* Drop HFXO before entering steady-state EM2 loop */
...
{
    /* Power: LP clock domain during EM2 (HFXO off, ~uA savings) */
    tarang_clock_revert_lp();

    EMU_EnterEM2(true);

    /* Wakeup: HF boost only for bounded compute + I2C/SPI */
...
/* ═══════════════════════════════════════════════════════════════════════════
 * sl_bt_on_event — anomaly-triggered telemetry hook (Kartik)
 * ═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════════════
 * Teammate stubs — replace with NLMS / TFLM / BLE implementations
 * ═══════════════════════════════════════════════════════════════════════════ */
...
static void tarang_event_dispatch(void)
{
    ...
    if (events & (uint32_t)TARANG_EVT_BLE) {
        tarang_clock_boost_hf();
        /* Kartik: sl_bt_advertiser_start / connection notify from here */
        tarang_clock_revert_lp();
    }
...
static bool ppg_burst_read_fifo(uint32_t *ppg_dst)
{
    ...
    /* FIFO overflow probe (OVF bit in status register) */
...
void tarang_ai_process(const tarang_ai_input_t *in)
{
    ...
    /* TFLM / MVP: tensors alias in->frame->data.* — token must match */
    (void)in;
}
