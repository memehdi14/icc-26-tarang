...
/* 1 = Bonded & Encrypted Mode (opt-in pairing via gateway TARANG_BLE_PAIR flag).
 * 0 = Direct Open Unbonded Mode (instant connection, no security). */
...
/* Bond epoch characteristic — MUST be generated by Simplicity Studio.
 * If this fires, you need to: (1) add the vitals_bond_epoch characteristic
 * to gatt_configuration.btconf, (2) regenerate via Studio, (3) rebuild. */
#ifndef gattdb_vitals_bond_epoch
  #error "gattdb_vitals_bond_epoch not defined — regenerate gatt_db.h in Simplicity Studio after adding the bond_epoch characteristic to gatt_configuration.btconf"
#endif
...
/* Bond epoch: NVM3-persisted counter, incremented when bonding table is
 * wiped (reflash or genuine key mismatch self-heal). Exposed as a plaintext
 * GATT characteristic so the gateway can detect stale bonds before trying
 * to resume encryption with an LTK that no longer exists on the pod. */
#define TARANG_NVM3_KEY_BOND_EPOCH  0x1000u
static uint32_t tarang_bond_epoch = 0u;

/* Tracks whether the current connection has already been secured (bonded).
 * Used to guard the bonding_failed handler: a redundant bond request on an
...
void tarang_ble_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (event_id) {
    ...
    case sl_bt_evt_system_boot_id:
    {
      ...
      /* SM flags = 0x0A:
       *   bit1 (0x02): encryption requires bonding → auto-enables bondable mode
       *   bit3 (0x08): emit confirm_bonding events → reject unexpected bonds
       * No separate sl_bt_sm_set_bondable_mode() needed — bit1 handles it. */
      sc = sl_bt_sm_configure(0x0A, sl_bt_sm_io_capability_noinputnooutput);
      tarang_ble_status_ok("configure security manager (bonded, Just-Works)", sc);
       ...
       * non-bondable — a phone's native Bluetooth pairing UI cannot
       * trigger a real bonding attempt against this pod. */
      sc = sl_bt_sm_configure(0x00, sl_bt_sm_io_capability_noinputnooutput);
      tarang_ble_status_ok("configure security manager (unbonded fallback)", sc);
      sc = sl_bt_sm_set_bondable_mode(0);
      tarang_ble_status_ok("disable bonding for direct-connect bring-up", sc);
  #endif

      /* ── Bond Epoch: detect NVM3 wipe (reflash) before Central tries stale LTK ── */
      {
        uint32_t num_bondings = 0;
        size_t bonding_handles_len = 0;
        sc = sl_bt_sm_get_bonding_handles(0, &num_bondings,
                                          0, &bonding_handles_len, NULL);

        Ecode_t nvm_sc = nvm3_readData(nvm3_defaultHandle,
                                       TARANG_NVM3_KEY_BOND_EPOCH,
                                       &tarang_bond_epoch,
                                       sizeof(tarang_bond_epoch));
        if (nvm_sc != ECODE_NVM3_OK) {
          tarang_bond_epoch = 0u;
        }
        ...
        if (num_bondings == 0u) {
          ...
          nvm3_writeData(nvm3_defaultHandle,
                         ...
                         &tarang_bond_epoch,
                         sizeof(tarang_bond_epoch));
          printf("[BLE][SM] Bond table empty (reflash?) — epoch incremented to %lu\r\n",
                 (unsigned long)tarang_bond_epoch);
        } else {
          printf("[BLE][SM] %lu existing bond(s) found, epoch=%lu\r\n",
                 (unsigned long)num_bondings, (unsigned long)tarang_bond_epoch);
        }
      }

      /* Write bond epoch to the plaintext GATT characteristic */
      sc = sl_bt_gatt_server_write_attribute_value(
          gattdb_vitals_bond_epoch, 0,
          sizeof(tarang_bond_epoch), (const uint8_t *)&tarang_bond_epoch);
      tarang_ble_status_ok("write bond epoch to GATT", sc);
#else
    ...
    /* ── Security Manager events — always handled when SM component is present ── */
#if defined(SL_CATALOG_BLUETOOTH_FEATURE_SM_PRESENT)
    ...
    case sl_bt_evt_sm_bonding_failed_id:
    {
      ...
      if (tarang_connection_was_secured) {
        /* Redundant bond request on an already-encrypted link.
         * Do NOT delete bonds — this is a collision, not a real key mismatch. */
        printf("[BLE][SM] Ignoring: link already secured, not deleting bonds.\r\n");
      } else {
        /* Genuine failure on a fresh/unsecured connection — self-heal. */
        printf("[BLE][SM] Genuine key mismatch — purging all pod bonds.\r\n");
        ...
        sl_bt_gatt_server_write_attribute_value(
            gattdb_vitals_bond_epoch, 0,
            sizeof(tarang_bond_epoch), (const uint8_t *)&tarang_bond_epoch);
        printf("[BLE][SM] Bond epoch incremented to %lu after self-heal purge.\r\n",
               (unsigned long)tarang_bond_epoch);
      }
    ...
    case sl_bt_evt_connection_closed_id:
    {
      ...
      if (reason == 0x0206 || reason == 0x0205 || reason == 0x001F) {
        ...
        sl_bt_gatt_server_write_attribute_value(
            gattdb_vitals_bond_epoch, 0,
            sizeof(tarang_bond_epoch), (const uint8_t *)&tarang_bond_epoch);
        printf("[BLE][SM] Purged bonds + epoch→%lu after auth failure (reason 0x%04X).\r\n",
               (unsigned long)tarang_bond_epoch, (unsigned)reason);
      }
#endif
